      ...
      <div className="grid grid-flow-row-dense gap-5 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {categories.map((category) => (
          <Link key={category} href={`/app/categories/${category}`}>
            <div className="grid h-64 place-items-center bg-white p-5 text-lg font-bold text-title shadow transition-opacity hover:bg-opacity-60 active:bg-opacity-90 md:text-xl">
              {formatEnum(category)}
            </div>
          </Link>
        ))}
