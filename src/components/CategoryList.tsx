        ...
        <div className="grid gap-5 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {categories.map((category) => (
            <Link key={category} href={`/app/categories/${category}`}>
              <div className="grid h-64 place-items-center bg-white text-lg font-bold capitalize text-title transition-opacity hover:bg-opacity-80 active:bg-opacity-100 md:text-xl">
                {category}
              </div>
            </Link>
          ))}
